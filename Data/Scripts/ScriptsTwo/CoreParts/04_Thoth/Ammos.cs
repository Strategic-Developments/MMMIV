...
{ // Don't edit above this line
    partial class Parts
    {
        private AmmoDef mss_lg_t_thoth_ammo => new AmmoDef // Your ID, for slotting into the Weapon CS
        {
            AmmoMagazine = "mss_lg_t_thoth_ammomagazine", // SubtypeId of physical ammo magazine. Use "Energy" for weapons without physical ammo.
            AmmoRound = "mss_lg_t_thoth_ammo", // Unique name used in server overrides and in the terminal (default).  Should be different for each ammoDef used by the same weapon.  Referred to for Shrapnel.
            ...
            {
                Patterns = Copy("mss_lg_t_thoth_ammo_vis", 119),
                Mode = Weapon, // Select when to activate this pattern, options:
        ...
        private AmmoDef mss_lg_t_thoth_ammo_vis => new AmmoDef // Your ID, for slotting into the Weapon CS
        {
            AmmoMagazine = "mss_lg_t_thoth_ammomagazine", // SubtypeId of physical ammo magazine. Use "Energy" for weapons without physical ammo.
            AmmoRound = "mss_lg_t_thoth_ammo_vis", // Unique name used in server overrides and in the terminal (default).  Should be different for each ammoDef used by the same weapon.  Referred to for Shrapnel.
            TerminalName = "JDDT", // Optional terminal name for this ammo type, used when picking ammo/cycling consumables.  Safe to have duplicates across different ammo defs.
            HybridRound = true, // Use both a physical ammo magazine and energy per shot.
            EnergyCost = 50, // Modified in MasterConfig here, actually just the power requirement needed to fire at 1.0 ROF scalar  If patterning ammos, only the main ammo (first fired) will count toward energy.
            BaseDamage = 1, // Direct damage; one steel plate is worth 100.
            ...
            {
                Enable = true, // Enable beam behaviour. Please have 3600 RPM, when this Setting is enabled. Please do not fire Beams into Voxels.
                VirtualBeams = true, // Only one damaging beam, but with the effectiveness of the visual beams combined (better performance).  If you are patterning a damage beam, ensure this is off for the non-AV beam
                ConvergeBeams = false, // When using virtual beams, converge the visual beams to the location of the real beam.
                RotateRealBeam = false, // The real beam is rotated between all visual beams, instead of centered between them.
                OneParticle = false, // Only spawn one particle hit per beam weapon.
                FakeVoxelHitTicks = 0, // If this beam hits/misses a voxel it assumes it will continue to do so for this many ticks at the same hit length and not extend further within this window.  This can save up to n times worth of cpu.
        ...
        private LineDef THOTH_FX => new LineDef
        {
            ...
            OffsetEffect = new OffsetEffectDef
            {
                ...
                MinLength = 125,
                MaxLength = 250, // MinLength and MaxLength determine the minimum and maximum length between segments.
