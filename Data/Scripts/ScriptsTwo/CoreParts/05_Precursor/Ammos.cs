...
namespace Scripts
{
    partial class Parts
    {
        public string[] MakeEntropicPattern()
        {
            string[] arr = new string[89];

            for (int i = 0; i < arr.Length; i++)
            {
                if (i < 60)
                    arr[i] = "mss_lg_t_entropic_ammo";
                else if (i >= 60 && i % 10 == 0)
                    arr[i] = "mss_lg_t_entropic_ammo_big";
                else
                    arr[i] = "mss_lg_t_entropic_ammo_blank";
            }
            return arr;
        }
        ...
        private AmmoDef mss_lg_t_entropic_ammo_big => new AmmoDef //Longest range, lowest dmg.
        {
            ...
            Health = 30, // 0 = disabled, otherwise how much damage it can take from other trajectiles before dying.
            BackKickForce = 0f,
            DecayPerShot = 0,
            EnergyMagazineSize = 90,
            IgnoreGrids = false,
            ...
            {
                AmmoRound = "mss_lg_t_entropic_ammo_big_expl", // AmmoRound field of the ammo to spawn.
            ...
            AmmoGraphics = new GraphicDef
            {
                ...
                Lines = new LineDef
                {
                    ...
                    OffsetEffect = new OffsetEffectDef
                    {
                        MaxOffset = 15,// 0 offset value disables this effect
        ...
        private AmmoDef mss_lg_t_entropic_ammo_big_expl => new AmmoDef // Your ID, for slotting into the Weapon CS
        {
            AmmoMagazine = "Energy", // SubtypeId of physical ammo magazine. Use "Energy" for weapons without physical ammo.
            AmmoRound = "mss_lg_t_entropic_ammo_big_expl", // Unique name used in server overrides and in the terminal (default).  Should be different for each ammoDef used by the same weapon.  Referred to for Shrapnel.
        ...
        private AmmoDef mss_lg_t_entropic_ammo_blank => new AmmoDef //Longest range, lowest dmg.
        {
            AmmoMagazine = "Energy",
            AmmoRound = "mss_lg_t_entropic_ammo_blank",
            HybridRound = false, //AmmoMagazine based weapon with energy cost
            EnergyCost = 600f, //(((EnergyCost * DefaultDamage) * ShotsPerSecond) * BarrelsPerShot) * ShotsPerBarrel
            BaseDamage = 0f,
            Mass = 0, // in kilograms
